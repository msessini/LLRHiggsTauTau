...
void HTauTauNtuplizer::analyze(const edm::Event& event, const edm::EventSetup& eSetup)
{
  ...
  Float_t antieweight=1;
  Float_t antimuweight=1;
  ...
  std::vector<pat::CompositeCandidate> candVectorSynch;
  ...
  for(edm::View<pat::CompositeCandidate>::const_iterator candi = cands->begin(); candi!=cands->end();++candi) {
    ...
    int decayModeFindingNewDMsSynch0=-1;
    int byMediumDeepTau2017v2p1VSjetSynch0 = -1, byVVLooseDeepTau2017v2p1VSeSynch0 = -1, byVLooseDeepTau2017v2p1VSmuSynch0 = -1;
    float decayModeSynch1=-1.;
    int decayModeFindingNewDMsSynch1=-1;
    int byMediumDeepTau2017v2p1VSjetSynch1 = -1, byVVLooseDeepTau2017v2p1VSeSynch1 = -1, byVLooseDeepTau2017v2p1VSmuSynch1 = -1;

    if(!candSynch.daughter(0)->isMuon() && !candSynch.daughter(0)->isElectron()){
      decayModeSynch0=userdatahelpers::getUserFloat (candSynch.daughter(0), "decayMode");
      decayModeFindingNewDMsSynch0=userdatahelpers::getUserInt (candSynch.daughter(0), "decayModeFindingNewDMs");
      Tau1P4Corrected=P4Corrected(candSynch.daughter(0)->p4(),GenMatch(candSynch.daughter(0), event),decayModeSynch0);
      //
      byMediumDeepTau2017v2p1VSjetSynch0 = userdatahelpers::getUserInt (candSynch.daughter(0), "byMediumDeepTau2017v2p1VSjet");
      byVVLooseDeepTau2017v2p1VSeSynch0 = userdatahelpers::getUserInt (candSynch.daughter(0), "byVVLooseDeepTau2017v2p1VSe");
      byVLooseDeepTau2017v2p1VSmuSynch0 = userdatahelpers::getUserInt (candSynch.daughter(0), "byVLooseDeepTau2017v2p1VSmu");
    }
    else Tau1P4Corrected=candSynch.daughter(0)->p4();
    if(!candSynch.daughter(1)->isMuon() && !candSynch.daughter(1)->isElectron()){
      ...
      Tau2P4Corrected=P4Corrected(candSynch.daughter(1)->p4(),GenMatch(candSynch.daughter(1), event),decayModeSynch1);
      //
    ...
    for(unsigned int i=0;i<LeptonP4.size();i++)
      {
	if(abs(deltaR(LeptonP4.at(i),candSynch.daughter(0)->p4()))<dRmin1){tau1Index=i;dRmin1=deltaR(LeptonP4.at(i),candSynch.daughter(0)->p4());}
	if(abs(deltaR(LeptonP4.at(i),candSynch.daughter(1)->p4()))<dRmin2){tau2Index=i;dRmin2=deltaR(LeptonP4.at(i),candSynch.daughter(1)->p4());}
      }
	...
	if(theisMC || (!theisMC && _runNumber>= 317509) )MatchedTriggerNames.push_back("HLT_DoubleMediumChargedIsoPFTauHPS35_Trk1_eta2p1_Reg_v");
	else if(!theisMC &&  _runNumber< 317509)
	  {
	    MatchedTriggerNames.push_back("HLT_DoubleMediumChargedIsoPFTau40_Trk1_TightID_eta2p1_Reg_v");
	    MatchedTriggerNames.push_back("HLT_DoubleTightChargedIsoPFTau40_Trk1_eta2p1_Reg_v");
	    MatchedTriggerNames.push_back("HLT_DoubleTightChargedIsoPFTau35_Trk1_TightID_eta2p1_Reg_v");
    ...
    const BXVector<l1t::Tau>* l1taus = L1TauHandle.product();
    bool L1Tau1=false,L1Tau2=false,L1=false;
	...
	for (BXVector<l1t::Tau>::const_iterator it=l1taus->begin(ibx); it!=l1taus->end(ibx); it++)
	  {
		...
		tlv_Tau.SetPtEtaPhiM(candSynch.daughter(0)->pt(),
				     candSynch.daughter(0)->eta(),
				     candSynch.daughter(0)->phi(),
				     0.);

		if ((tlv_L1Tau.DeltaR(tlv_Tau)*tlv_L1Tau.DeltaR(tlv_Tau)) < 0.25 && tlv_L1Tau.Pt()>32 && theYear==2017) L1Tau1 = true;
		else if((tlv_L1Tau.DeltaR(tlv_Tau)*tlv_L1Tau.DeltaR(tlv_Tau)) < 0.25 && theYear!=2017)  L1Tau1 = true;
	      }
	    }
	  }
      }
	...
	for (BXVector<l1t::Tau>::const_iterator it=l1taus->begin(ibx); it!=l1taus->end(ibx); it++)
	  {
		...
		tlv_Tau.SetPtEtaPhiM(candSynch.daughter(1)->pt(),
				     candSynch.daughter(1)->eta(),
				     candSynch.daughter(1)->phi(),
				     0.);

		if ((tlv_L1Tau.DeltaR(tlv_Tau)*tlv_L1Tau.DeltaR(tlv_Tau)) < 0.25 && tlv_L1Tau.Pt()>32 && theYear==2017) L1Tau2 = true;
		else if((tlv_L1Tau.DeltaR(tlv_Tau)*tlv_L1Tau.DeltaR(tlv_Tau)) < 0.25 && theYear!=2017)  L1Tau2 = true;
	      }
	    }
	  }
      }
    if (L1Tau1 && L1Tau2)L1=true;


   if(tau1Index!=-99 && tau2Index!=-99)
      {

	if((CHECK_BIT(_daughters_trgMatched.at(tau1Index),27) || CHECK_BIT(_daughters_trgMatched.at(tau1Index),29)) && (CHECK_BIT(_daughters_trgMatched.at(tau2Index),27) || CHECK_BIT(_daughters_trgMatched.at(tau2Index),29)) &&  trig && theYear==2016);
	else if((CHECK_BIT(_daughters_trgMatched.at(tau1Index),24) || CHECK_BIT(_daughters_trgMatched.at(tau1Index),26) || CHECK_BIT(_daughters_trgMatched.at(tau1Index),28)) && (CHECK_BIT(_daughters_trgMatched.at(tau2Index),24) || CHECK_BIT(_daughters_trgMatched.at(tau2Index),26) || CHECK_BIT(_daughters_trgMatched.at(tau2Index),28)) &&  trig && L1 && theYear==2017);
	else if(theYear==2018 && (theisMC || (!theisMC && !IsEmbed &&  _runNumber>= 317509)))
	  {
	    if(CHECK_BIT(_daughters_trgMatched.at(tau1Index),31) && CHECK_BIT(_daughters_trgMatched.at(tau2Index),31) &&  trig);
	    else continue;
	  }
	else if(theYear==2018 && !theisMC && !IsEmbed && _runNumber< 317509)
	  {
	    if((CHECK_BIT(_daughters_trgMatched.at(tau1Index),42) || CHECK_BIT(_daughters_trgMatched.at(tau1Index),44) || CHECK_BIT(_daughters_trgMatched.at(tau1Index),45)) && (CHECK_BIT(_daughters_trgMatched.at(tau2Index),42) || CHECK_BIT(_daughters_trgMatched.at(tau2Index),44) || CHECK_BIT(_daughters_trgMatched.at(tau2Index),45)) &&  trig);
	    else continue;
	  }
	else if(theYear==2018 && !theisMC && IsEmbed)
	  {
	    if((CHECK_BIT(_daughters_trgMatched.at(tau1Index),31)) && (CHECK_BIT(_daughters_trgMatched.at(tau2Index),31)));
	    else continue;
	  }
	else continue;
      }
    ...
    if (candSynch.daughter(0)->isMuon() || candSynch.daughter(0)->isElectron() ||candSynch.daughter(1)->isMuon() || candSynch.daughter(1)->isElectron())continue;
  ...
  for (int ipair=0;ipair<1;ipair++){
    ...
    if(!cand0Synch.daughter(0)->isMuon() && !cand0Synch.daughter(0)->isElectron()){
      byDeepTau2017v2p1VSjetraw_1.push_back(userdatahelpers::getUserFloat(cand0Synch.daughter(0),"byDeepTau2017v2p1VSjetraw"));
      byVVVLooseDeepTau2017v2p1VSjet_1.push_back(userdatahelpers::getUserInt (cand0Synch.daughter(0), "byVVVLooseDeepTau2017v2p1VSjet"));
      byVVLooseDeepTau2017v2p1VSjet_1.push_back(userdatahelpers::getUserInt (cand0Synch.daughter(0), "byVVLooseDeepTau2017v2p1VSjet"));
      byVLooseDeepTau2017v2p1VSjet_1.push_back(userdatahelpers::getUserInt (cand0Synch.daughter(0), "byVLooseDeepTau2017v2p1VSjet"));
      byLooseDeepTau2017v2p1VSjet_1.push_back(userdatahelpers::getUserInt (cand0Synch.daughter(0), "byLooseDeepTau2017v2p1VSjet"));
      byMediumDeepTau2017v2p1VSjet_1.push_back(userdatahelpers::getUserInt (cand0Synch.daughter(0), "byMediumDeepTau2017v2p1VSjet"));
      byTightDeepTau2017v2p1VSjet_1.push_back(userdatahelpers::getUserInt (cand0Synch.daughter(0), "byTightDeepTau2017v2p1VSjet"));
      byVTightDeepTau2017v2p1VSjet_1.push_back(userdatahelpers::getUserInt (cand0Synch.daughter(0), "byVTightDeepTau2017v2p1VSjet"));
      byVVTightDeepTau2017v2p1VSjet_1.push_back(userdatahelpers::getUserInt (cand0Synch.daughter(0), "byVVTightDeepTau2017v2p1VSjet"));

      byDeepTau2017v2p1VSeleraw_1.push_back(userdatahelpers::getUserFloat(cand0Synch.daughter(0),"byDeepTau2017v2p1VSeraw"));
      byVVVLooseDeepTau2017v2p1VSe_1.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(0),"byVVVLooseDeepTau2017v2p1VSe"));
      byVVLooseDeepTau2017v2p1VSe_1.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(0),"byVVLooseDeepTau2017v2p1VSe"));
      byVLooseDeepTau2017v2p1VSe_1.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(0),"byVLooseDeepTau2017v2p1VSe"));
      byLooseDeepTau2017v2p1VSe_1.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(0),"byLooseDeepTau2017v2p1VSe"));
      byMediumDeepTau2017v2p1VSe_1.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(0),"byMediumDeepTau2017v2p1VSe"));
      byTightDeepTau2017v2p1VSe_1.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(0),"byTightDeepTau2017v2p1VSe"));
      byVTightDeepTau2017v2p1VSe_1.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(0),"byVTightDeepTau2017v2p1VSe"));
      byVVTightDeepTau2017v2p1VSe_1.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(0),"byVVTightDeepTau2017v2p1VSe"));

      byDeepTau2017v2p1VSmuraw_1.push_back(userdatahelpers::getUserFloat(cand0Synch.daughter(0),"byDeepTau2017v2p1VSmuraw"));
      byVLooseDeepTau2017v2p1VSmu_1.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(0),"byVLooseDeepTau2017v2p1VSmu"));
      byLooseDeepTau2017v2p1VSmu_1.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(0),"byLooseDeepTau2017v2p1VSmu"));
      byMediumDeepTau2017v2p1VSmu_1.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(0),"byMediumDeepTau2017v2p1VSmu"));
      byTightDeepTau2017v2p1VSmu_1.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(0),"byTightDeepTau2017v2p1VSmu"));

      againstElectronVLooseMVA6_1.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(0),"againstElectronVLooseMVA6"));
      againstElectronLooseMVA6_1.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(0),"againstElectronLooseMVA6"));
      againstElectronMediumMVA6_1.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(0),"againstElectronMediumMVA6"));
      againstElectronTightMVA6_1.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(0),"againstElectronTightMVA6"));
      againstElectronVTightMVA6_1.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(0),"againstElectronVTightMVA6"));
      againstMuonLoose3_1.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(0),"againstMuonLoose3"));
      againstMuonTight3_1.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(0),"againstMuonTight3"));
      byIsolationMVA3oldDMwLTraw_1.push_back(userdatahelpers::getUserFloat(cand0Synch.daughter(0),"byIsolationMVArun2017v2DBoldDMdR0p3wLTraw2017"));
    }
    else
      {
	byDeepTau2017v2p1VSjetraw_1.push_back(false);
	byVVVLooseDeepTau2017v2p1VSjet_1.push_back(false);
	byVVLooseDeepTau2017v2p1VSjet_1.push_back(false);
	byVLooseDeepTau2017v2p1VSjet_1.push_back(false);
	byLooseDeepTau2017v2p1VSjet_1.push_back(false);
	byMediumDeepTau2017v2p1VSjet_1.push_back(false);
	byTightDeepTau2017v2p1VSjet_1.push_back(false);
	byVTightDeepTau2017v2p1VSjet_1.push_back(false);
	byVVTightDeepTau2017v2p1VSjet_1.push_back(false);

	byDeepTau2017v2p1VSeleraw_1.push_back(false);
	byVVVLooseDeepTau2017v2p1VSe_1.push_back(false);
	byVVLooseDeepTau2017v2p1VSe_1.push_back(false);
	byVLooseDeepTau2017v2p1VSe_1.push_back(false);
	byLooseDeepTau2017v2p1VSe_1.push_back(false);
	byMediumDeepTau2017v2p1VSe_1.push_back(false);
	byTightDeepTau2017v2p1VSe_1.push_back(false);
	byVTightDeepTau2017v2p1VSe_1.push_back(false);
	byVVTightDeepTau2017v2p1VSe_1.push_back(false);

	byDeepTau2017v2p1VSmuraw_1.push_back(false);
	byVLooseDeepTau2017v2p1VSmu_1.push_back(false);
	byLooseDeepTau2017v2p1VSmu_1.push_back(false);
	byMediumDeepTau2017v2p1VSmu_1.push_back(false);
	byTightDeepTau2017v2p1VSmu_1.push_back(false);

	againstElectronVLooseMVA6_1.push_back(false);
	againstElectronLooseMVA6_1.push_back(false);
	againstElectronMediumMVA6_1.push_back(false);
	againstElectronTightMVA6_1.push_back(false);
	againstElectronVTightMVA6_1.push_back(false);
	againstMuonLoose3_1.push_back(false);
	againstMuonTight3_1.push_back(false);
	byIsolationMVA3oldDMwLTraw_1.push_back(false);
	Tau1P4Corrected=cand0Synch.daughter(0)->p4();
      }
    if(!cand0Synch.daughter(1)->isMuon() && !cand0Synch.daughter(1)->isElectron()){
      byDeepTau2017v2p1VSjetraw_2.push_back(userdatahelpers::getUserFloat(cand0Synch.daughter(1),"byDeepTau2017v2p1VSjetraw"));
      byVVVLooseDeepTau2017v2p1VSjet_2.push_back(userdatahelpers::getUserInt (cand0Synch.daughter(1), "byVVVLooseDeepTau2017v2p1VSjet"));
      byVVLooseDeepTau2017v2p1VSjet_2.push_back(userdatahelpers::getUserInt (cand0Synch.daughter(1), "byVVLooseDeepTau2017v2p1VSjet"));
      byVLooseDeepTau2017v2p1VSjet_2.push_back(userdatahelpers::getUserInt (cand0Synch.daughter(1), "byVLooseDeepTau2017v2p1VSjet"));
      byLooseDeepTau2017v2p1VSjet_2.push_back(userdatahelpers::getUserInt (cand0Synch.daughter(1), "byLooseDeepTau2017v2p1VSjet"));
      byMediumDeepTau2017v2p1VSjet_2.push_back(userdatahelpers::getUserInt (cand0Synch.daughter(1), "byMediumDeepTau2017v2p1VSjet"));
      byTightDeepTau2017v2p1VSjet_2.push_back(userdatahelpers::getUserInt (cand0Synch.daughter(1), "byTightDeepTau2017v2p1VSjet"));
      byVTightDeepTau2017v2p1VSjet_2.push_back(userdatahelpers::getUserInt (cand0Synch.daughter(1), "byVTightDeepTau2017v2p1VSjet"));
      byVVTightDeepTau2017v2p1VSjet_2.push_back(userdatahelpers::getUserInt (cand0Synch.daughter(1), "byVVTightDeepTau2017v2p1VSjet"));

      byDeepTau2017v2p1VSeleraw_2.push_back(userdatahelpers::getUserFloat(cand0Synch.daughter(1),"byDeepTau2017v2p1VSeraw"));
      byVVVLooseDeepTau2017v2p1VSe_2.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(1),"byVVVLooseDeepTau2017v2p1VSe"));
      byVVLooseDeepTau2017v2p1VSe_2.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(1),"byVVLooseDeepTau2017v2p1VSe"));
      byVLooseDeepTau2017v2p1VSe_2.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(1),"byVLooseDeepTau2017v2p1VSe"));
      byLooseDeepTau2017v2p1VSe_2.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(1),"byLooseDeepTau2017v2p1VSe"));
      byMediumDeepTau2017v2p1VSe_2.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(1),"byMediumDeepTau2017v2p1VSe"));
      byTightDeepTau2017v2p1VSe_2.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(1),"byTightDeepTau2017v2p1VSe"));
      byVTightDeepTau2017v2p1VSe_2.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(1),"byVTightDeepTau2017v2p1VSe"));
      byVVTightDeepTau2017v2p1VSe_2.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(1),"byVVTightDeepTau2017v2p1VSe"));

      byDeepTau2017v2p1VSmuraw_2.push_back(userdatahelpers::getUserFloat(cand0Synch.daughter(1),"byDeepTau2017v2p1VSmuraw"));
      byVLooseDeepTau2017v2p1VSmu_2.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(1),"byVLooseDeepTau2017v2p1VSmu"));
      byLooseDeepTau2017v2p1VSmu_2.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(1),"byLooseDeepTau2017v2p1VSmu"));
      byMediumDeepTau2017v2p1VSmu_2.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(1),"byMediumDeepTau2017v2p1VSmu"));
      byTightDeepTau2017v2p1VSmu_2.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(1),"byTightDeepTau2017v2p1VSmu"));

      againstElectronVLooseMVA6_2.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(1),"againstElectronVLooseMVA6"));
      againstElectronLooseMVA6_2.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(1),"againstElectronLooseMVA6"));
      againstElectronMediumMVA6_2.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(1),"againstElectronMediumMVA6"));
      againstElectronTightMVA6_2.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(1),"againstElectronTightMVA6"));
      againstElectronVTightMVA6_2.push_back(userdatahelpers::getUserInt(cand0Synch.daughter(1),"againstElectronVTightMVA6"));
      ...
      {
	byDeepTau2017v2p1VSjetraw_2.push_back(false);
	byVVVLooseDeepTau2017v2p1VSjet_2.push_back(false);
	byVVLooseDeepTau2017v2p1VSjet_2.push_back(false);
	byVLooseDeepTau2017v2p1VSjet_2.push_back(false);
	byLooseDeepTau2017v2p1VSjet_2.push_back(false);
	byMediumDeepTau2017v2p1VSjet_2.push_back(false);
	byTightDeepTau2017v2p1VSjet_2.push_back(false);
	byVTightDeepTau2017v2p1VSjet_2.push_back(false);
	byVVTightDeepTau2017v2p1VSjet_2.push_back(false);

	byDeepTau2017v2p1VSeleraw_2.push_back(false);
	byVVVLooseDeepTau2017v2p1VSe_2.push_back(false);
	byVVLooseDeepTau2017v2p1VSe_2.push_back(false);
	byVLooseDeepTau2017v2p1VSe_2.push_back(false);
	byLooseDeepTau2017v2p1VSe_2.push_back(false);
	byMediumDeepTau2017v2p1VSe_2.push_back(false);
	byTightDeepTau2017v2p1VSe_2.push_back(false);
	byVTightDeepTau2017v2p1VSe_2.push_back(false);
	byVVTightDeepTau2017v2p1VSe_2.push_back(false);

	byDeepTau2017v2p1VSmuraw_2.push_back(false);
	byVLooseDeepTau2017v2p1VSmu_2.push_back(false);
	byLooseDeepTau2017v2p1VSmu_2.push_back(false);
	byMediumDeepTau2017v2p1VSmu_2.push_back(false);
	byTightDeepTau2017v2p1VSmu_2.push_back(false);

	againstElectronVLooseMVA6_2.push_back(false);
	againstElectronLooseMVA6_2.push_back(false);
	againstElectronMediumMVA6_2.push_back(false);
	againstElectronTightMVA6_2.push_back(false);
	againstElectronVTightMVA6_2.push_back(false);
	againstMuonLoose3_2.push_back(false);
	againstMuonTight3_2.push_back(false);
    ...
    if(cand0Synch.daughter(1)->isMuon()){dm_2.push_back(16);dmMVA_2.push_back(15);}
    else if (cand0Synch.daughter(1)->isElectron()){dm_2.push_back(15);dmMVA_2.push_back(15);}
    else {dm_2.push_back(userdatahelpers::getUserFloat (cand0Synch.daughter(1), "decayMode"));dmMVA_2.push_back(userdatahelpers::getUserFloat(cand0Synch.daughter(1),"MVADM2017v1"));}
    ...
    for(unsigned int i=0;i<LeptonP4.size();i++)
      {
	if(abs(deltaR(LeptonP4.at(i),cand0Synch.daughter(0)->p4()))<dRmin1){tau1Index=i;dRmin1=deltaR(LeptonP4.at(i),cand0Synch.daughter(0)->p4());}
	if(abs(deltaR(LeptonP4.at(i),cand0Synch.daughter(1)->p4()))<dRmin2){tau2Index=i;dRmin2=deltaR(LeptonP4.at(i),cand0Synch.daughter(1)->p4());}
      }
    ...
    m_1.push_back(Tau1P4Corrected.M());
    q_1.push_back(cand0Synch.daughter(0)->charge());
    d0_1.push_back(userdatahelpers::getUserFloat(cand0Synch.daughter(0),"dxy"));
    dz_1.push_back(userdatahelpers::getUserFloat(cand0Synch.daughter(0),"dz"));
    mt_1.push_back(ComputeMT(Tau1P4Corrected,cand0Synch.userFloat("MEt_px"), cand0Synch.userFloat("MEt_py")));
    puppimt_1.push_back(ComputePUPPIMT(Tau1P4Corrected,_PUPPImetphi,_PUPPImet));
    iso_1.push_back(byDeepTau2017v2p1VSjetraw_1.at(ipair));
    if(theisMC || IsEmbed){
      if(tau1Index!=-99)gen_match_1.push_back(GenMatch(cand0Synch.daughter(0), event));
      else gen_match_1.push_back(-99);
    }
    againstElectronLooseMVA6_1.push_back(againstElectronLooseMVA6_1.at(ipair));
    againstElectronMediumMVA6_1.push_back(againstElectronMediumMVA6_1.at(ipair));
    againstElectronTightMVA6_1.push_back(againstElectronTightMVA6_1.at(ipair));
    againstElectronVLooseMVA6_1.push_back(againstElectronVLooseMVA6_1.at(ipair));
    againstElectronVTightMVA6_1.push_back(againstElectronVTightMVA6_1.at(ipair));
    againstMuonLoose3_1.push_back(againstMuonLoose3_1.at(ipair));
    againstMuonTight3_1.push_back(againstMuonTight3_1.at(ipair));

    if(theisMC){
      if(theYear==2016)
	{
	  idisoweight_1.push_back(tauSFTool2016->getSFvsDM(pt_1.at(ipair),dm_1.at(ipair),gen_match_1.at(ipair)));
	  antieweight_1.push_back(antiEleSFTool2016->getSFvsEta(eta_1.at(ipair),gen_match_1.at(ipair)));
	  antimuweight_1.push_back(antiMuSFTool2016->getSFvsEta(eta_1.at(ipair),gen_match_1.at(ipair)));
	}
      if(theYear==2017)
	{
	  idisoweight_1.push_back(tauSFTool2017->getSFvsDM(pt_1.at(ipair),dm_1.at(ipair),gen_match_1.at(ipair)));
	  antieweight_1.push_back(antiEleSFTool2017->getSFvsEta(eta_1.at(ipair),gen_match_1.at(ipair)));
	  antimuweight_1.push_back(antiMuSFTool2017->getSFvsEta(eta_1.at(ipair),gen_match_1.at(ipair)));
	}
      if(theYear==2018)
	{
	  idisoweight_1.push_back(tauSFTool2018->getSFvsDM(pt_1.at(ipair),dm_1.at(ipair),gen_match_1.at(ipair)));
	  antieweight_1.push_back(antiEleSFTool2018->getSFvsEta(eta_1.at(ipair),gen_match_1.at(ipair)));
	  antimuweight_1.push_back(antiMuSFTool2018->getSFvsEta(eta_1.at(ipair),gen_match_1.at(ipair)));
	}
    }
    ...
    m_2.push_back(Tau2P4Corrected.M());
    q_2.push_back(cand0Synch.daughter(1)->charge());
    d0_2.push_back(userdatahelpers::getUserFloat(cand0Synch.daughter(1),"dxy"));
    dz_2.push_back(userdatahelpers::getUserFloat(cand0Synch.daughter(1),"dz"));
    mt_2.push_back(ComputeMT(Tau2P4Corrected,cand0Synch.userFloat("MEt_px"), cand0Synch.userFloat("MEt_py")));
    puppimt_2.push_back(ComputePUPPIMT(Tau2P4Corrected,_PUPPImetphi,_PUPPImet));

    iso_2.push_back(byDeepTau2017v2p1VSjetraw_2.at(ipair));

    if(theisMC || IsEmbed){
      if(tau2Index!=-99)gen_match_2.push_back( GenMatch(cand0Synch.daughter(1), event));
      else gen_match_2.push_back(-99);
    }
    againstElectronLooseMVA6_2.push_back(againstElectronLooseMVA6_2.at(ipair));
    againstElectronMediumMVA6_2.push_back(againstElectronMediumMVA6_2.at(ipair));
    againstElectronTightMVA6_2.push_back(againstElectronTightMVA6_2.at(ipair));
    againstElectronVLooseMVA6_2.push_back(againstElectronVLooseMVA6_2.at(ipair));
    againstElectronVTightMVA6_2.push_back(againstElectronVTightMVA6_2.at(ipair));
    againstMuonLoose3_2.push_back(againstMuonLoose3_2.at(ipair));
    againstMuonTight3_2.push_back(againstMuonTight3_2.at(ipair));

    if(theisMC) {
      if(theYear==2016)
	{
	  idisoweight_2.push_back(tauSFTool2016->getSFvsDM(pt_2.at(ipair),dm_2.at(ipair),gen_match_2.at(ipair)));
	  antieweight_2.push_back(antiEleSFTool2016->getSFvsEta(eta_2.at(ipair),gen_match_2.at(ipair)));
	  antimuweight_2.push_back(antiMuSFTool2016->getSFvsEta(eta_2.at(ipair),gen_match_2.at(ipair)));
	}
      if(theYear==2017)
	{
	  idisoweight_2.push_back(tauSFTool2017->getSFvsDM(pt_2.at(ipair),dm_2.at(ipair),gen_match_2.at(ipair)));
	  antieweight_2.push_back(antiEleSFTool2017->getSFvsEta(eta_2.at(ipair),gen_match_2.at(ipair)));
	  antimuweight_2.push_back(antiMuSFTool2017->getSFvsEta(eta_2.at(ipair),gen_match_2.at(ipair)));
	}
      if(theYear==2018)
	{
	  idisoweight_2.push_back(tauSFTool2018->getSFvsDM(pt_2.at(ipair),dm_2.at(ipair),gen_match_2.at(ipair)));
	  antieweight_2.push_back(antiEleSFTool2018->getSFvsEta(eta_2.at(ipair),gen_match_2.at(ipair)));
	  antimuweight_2.push_back(antiMuSFTool2018->getSFvsEta(eta_2.at(ipair),gen_match_2.at(ipair)));
	}
      idisoweight=idisoweight_1.at(ipair)*idisoweight_2.at(ipair);
      antieweight=antieweight_1.at(ipair)*antieweight_2.at(ipair);
      antimuweight=antimuweight_1.at(ipair)*antimuweight_2.at(ipair);
	...
	for (unsigned int ivertex =0; ivertex<_VertexHashBS1.size(); ivertex++){
	  ...
	  if ( std::find(hashes.begin(), hashes.end(), selectionHash) != hashes.end() ){
	    pvx.push_back(_RefitPVBS_x.at(ivertex));
	    pvy.push_back(_RefitPVBS_y.at(ivertex));
    ...
    if(tau1Index!=-99 && tau2Index!=-99 )
      {
	if(_PFTauSVPos.at(tau1Index).size()==3 && dmMVA_1.at(ipair)==10)
	  {
	    svx_1.push_back(_PFTauSVPos.at(tau1Index).at(0));
	    svy_1.push_back(_PFTauSVPos.at(tau1Index).at(1));
	    svz_1.push_back(_PFTauSVPos.at(tau1Index).at(2));
	  }
	if(_PFTauSVPos.at(tau2Index).size()==3 && dmMVA_2.at(ipair)==10)
	  {
	    svx_2.push_back(_PFTauSVPos.at(tau2Index).at(0));
	    svy_2.push_back(_PFTauSVPos.at(tau2Index).at(1));
	    svz_2.push_back(_PFTauSVPos.at(tau2Index).at(2));
	  }
      }

    byDeepTau2017v2p1VSjetraw_1.push_back(byDeepTau2017v2p1VSjetraw_1.at(ipair));
    byDeepTau2017v2p1VSeleraw_1.push_back(byDeepTau2017v2p1VSeleraw_1.at(ipair));
    byDeepTau2017v2p1VSmuraw_1.push_back(byDeepTau2017v2p1VSmuraw_1.at(ipair));

    byDeepTau2017v2p1VSjetraw_2.push_back(byDeepTau2017v2p1VSjetraw_2.at(ipair));
    byDeepTau2017v2p1VSeleraw_2.push_back(byDeepTau2017v2p1VSeleraw_2.at(ipair));
    byDeepTau2017v2p1VSmuraw_2.push_back(byDeepTau2017v2p1VSmuraw_2.at(ipair));

    pt_tt.push_back((Tau1P4Corrected+Tau2P4Corrected+PUPPImet.p4()).pt());
    pt_vis.push_back((Tau1P4Corrected+Tau2P4Corrected).pt());
    mt_tot.push_back(ComputeMTtot(Tau1P4Corrected,Tau2P4Corrected,PUPPImet.p4().X(),PUPPImet.p4().Y() ));
    m_vis.push_back(((Tau1P4Corrected+Tau2P4Corrected).M()));


    float thisMETpx = cand0Synch.userFloat("MEt_px");
    float thisMETpy = cand0Synch.userFloat("MEt_py");

    _metx.push_back(thisMETpx);
    _mety.push_back(thisMETpy);


    pzetavis .push_back( 1./(sqrt(pow((cos(phi_2.at(ipair))+cos(phi_1.at(ipair))),2)+pow((sin(phi_2.at(ipair))+sin(phi_1.at(ipair))),2))) * ((Tau2P4Corrected.Px()+Tau1P4Corrected.Px())*(cos(phi_2.at(ipair))+cos(phi_1.at(ipair))) + (sin(phi_2.at(ipair))+sin(phi_1.at(ipair)))*(Tau2P4Corrected.Py()+Tau1P4Corrected.Py())));
    pzetamiss.push_back( 1./(sqrt(pow((cos(phi_2.at(ipair))+cos(phi_1.at(ipair))),2)+pow((sin(phi_2.at(ipair))+sin(phi_1.at(ipair))),2))) * (cand0Synch.userFloat("MEt_px")*(cos(phi_2.at(ipair))+cos(phi_1.at(ipair))) + (sin(phi_2.at(ipair))+sin(phi_1.at(ipair)))*cand0Synch.userFloat("MEt_py")));
    puppipzetamiss.push_back(1./(sqrt(pow((cos(phi_2.at(ipair))+cos(phi_1.at(ipair))),2)+pow((sin(phi_2.at(ipair))+sin(phi_1.at(ipair))),2))) * (PUPPImet.px()*(cos(phi_2.at(ipair))+cos(phi_1.at(ipair))) + (sin(phi_2.at(ipair))+sin(phi_1.at(ipair)))*PUPPImet.py()));

    if(theisMC)weight.push_back(idisoweight*puweight*antieweight*antimuweight);


    const pat::Tau *taon1=dynamic_cast<const pat::Tau*>(cand0Synch.daughter(0));
    const pat::Tau *taon2=dynamic_cast<const pat::Tau*>(cand0Synch.daughter(1));

    math::XYZTLorentzVector HadP41;
    math::XYZTLorentzVector HadP42;
    math::XYZTLorentzVector nHadP41;
    math::XYZTLorentzVector nHadP42;
    ...
    if(taon1)
      {
	reco::CandidatePtrVector chCands1Synch = taon1->signalChargedHadrCands();
	reco::CandidatePtrVector neCands1Synch = taon1->signalGammaCands();
	chCands1Synch = taon1->signalChargedHadrCands();
	neCands1Synch= taon1->signalGammaCands();

	for(reco::CandidatePtrVector::const_iterator id=chCands1Synch.begin();id!=chCands1Synch.end(); ++id)
	  {
	    HadP41+=(*id)->p4();//*tes1;
	    chpt_1 += (*id)->p4().pt();//*tes1;
	    chphi_1 +=(*id)->p4().phi();
	    cheta_1 += (*id)->p4().eta();
	    chm_1 += (*id)->p4().M();//*tes1;
	  }

	nHadP41=Tau1P4Corrected-HadP41;
	_npt_1.push_back( nHadP41.pt());//*tes1;
	_nphi_1.push_back( nHadP41.phi());
	_neta_1.push_back( nHadP41.eta());
	_nm_1.push_back( nHadP41.M());//*tes1;

      }
    else {
      chpt_1 += pt_1.at(ipair);
      chphi_1 += phi_1.at(ipair);
      cheta_1 += eta_1.at(ipair);
      chm_1 += m_1.at(ipair);
    }
    _chpt_1.push_back(chpt_1);
    _chphi_1.push_back(chphi_1);
    _cheta_1.push_back(cheta_1);
    _chm_1.push_back(chm_1);

    if(taon2){
      reco::CandidatePtrVector chCands2Synch = taon2->signalChargedHadrCands();
      reco::CandidatePtrVector neCands2Synch = taon2->signalGammaCands();
      chCands2Synch = taon2->signalChargedHadrCands();
      neCands2Synch= taon2->signalGammaCands();
      for(reco::CandidatePtrVector::const_iterator id=chCands2Synch.begin();id!=chCands2Synch.end(); ++id)
	{
	  HadP42 +=(*id)->p4();//*tes2;
	  chpt_2 += (*id)->p4().pt();//*tes2;
	  chphi_2 +=(*id)->p4().phi();
	  cheta_2 += (*id)->p4().eta();
	  chm_2 += (*id)->p4().M();//*tes2;
	}

      nHadP42=Tau2P4Corrected-HadP42;
      _npt_2.push_back(nHadP42.pt());//*tes2;
      _nphi_2.push_back(nHadP42.phi());
      _neta_2.push_back(nHadP42.eta());
      _nm_2.push_back(nHadP42.M());//*tes2;


    }
    else{
      chpt_2 += pt_2.at(ipair);
      chphi_2 +=phi_2.at(ipair);
      cheta_2+= eta_2.at(ipair);
      chm_2+= m_2.at(ipair);
    }

    _chpt_2.push_back(chpt_2);
    _chphi_2.push_back(chphi_2);
    _cheta_2.push_back(cheta_2);
    _chm_2.push_back(chm_2);

    //_numberOfJets = 0;
    if(writeJets){
      njets.push_back(FillJet(Smearedjets, SmearedjetsDown, SmearedjetsUp, event, eSetup, &jecUnc, &jecSourceUncProviders,&jecSourceUncRegroupedProviders,cand0Synch.daughter(0),cand0Synch.daughter(1),ipair));
	...
	if(writeJets){
	  if(computeQGVar){ //Needs jetHandle + qgTaggerHandle
	    for(auto jet = SmearedjetHandle->begin(); jet != SmearedjetHandle->end(); ++jet){
	      edm::RefToBase<pat::Jet> jetRef(edm::Ref<edm::View<pat::Jet> >(SmearedjetHandle, jet - SmearedjetHandle->begin()));
...
int HTauTauNtuplizer::FillJet(const edm::View<pat::Jet> *jets, const edm::View<pat::Jet> *jetsDown,const edm::View<pat::Jet> *jetsUp,const edm::Event& event, edm::EventSetup const& iSetup, JetCorrectionUncertainty* jecUnc, myJECMap* jecSourceUncProviders, myJECMap* jecSourceUncRegroupedProviders,const reco::Candidate* cand1,const reco::Candidate* cand2, int ipair) {
  ...
  for(edm::View<pat::Jet>::const_iterator ijet = jets->begin(); ijet!=jets->end();++ijet) {
    ...
    if(ijet->pt()<20 || abs(ijet->eta())>4.7 || deltaR(ijet->p4(),cand1->p4())<0.5 || deltaR(ijet->p4(),cand2->p4())<0.5)continue;
...
math::XYZTLorentzVector HTauTauNtuplizer::P4Corrected(math::XYZTLorentzVector p4, int genmatch, int DM){
  ...
  if(theYear==2016){
    Int_t binTES = histTES2016->GetXaxis()->FindBin(DM);

    if(DM==0)NomTESCorrDM0=histTES2016->GetBinContent(binTES);
    if(DM==1)NomTESCorrDM1=histTES2016->GetBinContent(binTES);
    if(DM==10)NomTESCorrDM10=histTES2016->GetBinContent(binTES);
    if(DM==11)NomTESCorrDM11=histTES2016->GetBinContent(binTES);
    if(DM==0)UncTESCorrDM0=histTES2016->GetBinError(binTES);
    if(DM==1)UncTESCorrDM1=histTES2016->GetBinError(binTES);
    if(DM==10)UncTESCorrDM10=histTES2016->GetBinError(binTES);
    if(DM==11)UncTESCorrDM11=histTES2016->GetBinError(binTES);

  }
  else if(theYear==2017){
    Int_t binTES = histTES2017->GetXaxis()->FindBin(DM);

    if(DM==0)NomTESCorrDM0=histTES2017->GetBinContent(binTES);
    if(DM==1)NomTESCorrDM1=histTES2017->GetBinContent(binTES);
    if(DM==10)NomTESCorrDM10=histTES2017->GetBinContent(binTES);
    if(DM==11)NomTESCorrDM11=histTES2017->GetBinContent(binTES);
    if(DM==0)UncTESCorrDM0=histTES2017->GetBinError(binTES);
    if(DM==1)UncTESCorrDM1=histTES2017->GetBinError(binTES);
    if(DM==10)UncTESCorrDM10=histTES2017->GetBinError(binTES);
    if(DM==11)UncTESCorrDM11=histTES2017->GetBinError(binTES);

  }
  else if(theYear==2018){
    Int_t binTES = histTES2018->GetXaxis()->FindBin(DM);

    if(DM==0)NomTESCorrDM0=histTES2018->GetBinContent(binTES);
    if(DM==1)NomTESCorrDM1=histTES2018->GetBinContent(binTES);
    if(DM==10)NomTESCorrDM10=histTES2018->GetBinContent(binTES);
    if(DM==11)NomTESCorrDM11=histTES2018->GetBinContent(binTES);
    if(DM==0)UncTESCorrDM0=histTES2018->GetBinError(binTES);
    if(DM==1)UncTESCorrDM1=histTES2018->GetBinError(binTES);
    if(DM==10)UncTESCorrDM10=histTES2018->GetBinError(binTES);
    if(DM==11)UncTESCorrDM11=histTES2018->GetBinError(binTES);
  ...
  if(genmatch==5)
      ...
      if (DM==0) // 1prong
	{
      ...
      else if ( (DM==1) || (DM==2) )  // 1prong+pi0 or 1prong+2pi0
	{
	  shiftP    = Shift1PrPi0;
	  shiftMass = Shift1PrPi0;
	}
      else if (DM==10) // 3prong
	{
	  shiftP    = Shift3Pr;
	  shiftMass = Shift3Pr;
	}
      else if (DM==11) // 3prong+pi0
	{
	  shiftP    = Shift3PrPi0;
	  shiftMass = Shift3PrPi0;
	}
    }
  if ((genmatch == 1 || genmatch == 3) && DM==0)  {
    shiftP    = EFakeShift1PrB;     // 1prong
    if (fabs(p4.Eta())> 1.479) shiftP    = EFakeShift1PrE;
    shiftMass = 1.;
  }
  if ((genmatch == 1 || genmatch == 3) && DM==1) {
    shiftP    = EFakeShift1PrPi0B;  // 1prong+pi0
    shiftMass = EFakeShift1PrPi0B;
    if (fabs(p4.Eta())> 1.479) {
      shiftP    = EFakeShift1PrPi0E;
      shiftMass = EFakeShift1PrPi0E;
    }
  }

  double pxS_Nominal = p4.Px()*shiftP;
  double pyS_Nominal = p4.Py()*shiftP;
  double pzS_Nominal = p4.Pz()*shiftP;
  double massS_Nominal = p4.M()*shiftMass;
  ...
  return math::XYZTLorentzVector(pxS_Nominal, pyS_Nominal, pzS_Nominal, enS_Nominal);
}
...
bool HTauTauNtuplizer::refitPV(const edm::Event & iEvent, const edm::EventSetup & iSetup){
  ...
  if(fitOk) {
    ...
    for (int i = 0; i < 3; i++) {
      for (int j = i; j < 3; j++) {
	_pvRefit_cov.push_back(pvcov(i, j));
      }
    }
  }
  else {
